...
int main(int argc, char *argv[])
{
  ...
  auto const node = std::make_shared<rclcpp::Node>(
      "show_selected_grasps", rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true));
  ...
  for (int j = 0; j < 6; j++)
  {
    ...
    if (j == 0)
    {
      T_face = Eigen::Isometry3d::Identity();
    }
    else if (j == 5)
    {
      T_face = (Eigen::AngleAxisd(M_PI, Eigen::Vector3d::UnitX())) * Eigen::Isometry3d::Identity();
    }
    else
    {
      T_face = (Eigen::AngleAxisd(M_PI / 2.0, Eigen::Vector3d::UnitX())) * (Eigen::AngleAxisd(M_PI / 2.0 * (j - 1), Eigen::Vector3d::UnitY())) * Eigen::Isometry3d::Identity();
    }
    for (int i = 0; i < 12; i++)
    {
      if (i < 4)
      ...
      T_Egr_B = cube_pose * T_face * T_Ogr_O * T_rest;
      T_Epre_B = T_Egr_B * Eigen::Translation3d(-0.1, 0.0, 0.0);

      double z_Egr_B = T_Egr_B.translation().z();
      double z_Epre_B = T_Epre_B.translation().z();

      if (z_Egr_B + 0.0001 >= z_Epre_B)
      {
        continue;
      }

      moveit_msgs::msg::Grasp vis_gr;
      vis_gr.id = "grasp_" + std::to_string(i + 1) + "_" + std::to_string(j + 1);
      ...
      RCLCPP_INFO(logger, "This is grasp no. %d for face no. %d - it is up-directed grasp", i + 1, j + 1);
