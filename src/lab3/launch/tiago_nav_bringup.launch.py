...
def public_nav_function(context, *args, **kwargs):
    ...
    actions = []
    tiago_2dnav = get_package_share_directory("lab3")
    param_file = os.path.join(tiago_2dnav, "params", "tiago_" + base_type + "_nav_public_sim.yaml")

    pal_maps = get_package_share_directory("lab3")

    map_path = os.path.join(pal_maps, "stero_world" , "map.yaml")
    rviz_config_file = os.path.join(tiago_2dnav, "config", "config.rviz")
